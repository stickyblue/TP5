...
// Class describing a BigIndustrialLogisticUser, 
// A BigIndustrialLogisticUser prefers to use Big Plane, Heavy Train and Heavy Boat whenever possible, and Heavy Truc (Road) Otherwise only when it is absolutely needed.
// A BigIndustrialLogisticUser evaluate the cost of a traject by considering the transportation time of the final path it selects.
class BigIndustrialLogisticUser: public User{
    ...
    public:
        ...
        // Initialize the acceptable_mode to ["Train", "Plane","Boat"]
        BigIndustrialLogisticUser(std::string n, CityPtr c, WorldMap& m);
