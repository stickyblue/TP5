...
// Class describing an EcoUser, 
// An EcoUser who prefers to use train when possible, and car otherwise only when absolutely needed.
// An EcoUser evaluate the cost of a traject by considering the ecological impact in CO2eq of the final path it selects.
class EcoUser: public User{
    ...
    protected:
        ...
        std::vector<std::string> acceptable_mode  = {"road", "train"};

    public:

        // Constructor of User
        // @param n the user name, c a shared_ptr onto a City, m a reference onto a WorldMap
        // Initialize the acceptable_mode to ["train"]
        EcoUser(std::string n, CityPtr c, WorldMap& m);
